...
function calculateDamage(item, idx) {
    ...
    if (gC('b_bj') && parseInt(gS('bj_const')) >= 2) { cd += 24; cd_list.push("扳机2命(24%)"); }
    ...
    if (gS('y_wp') === 'jq') {
        ...
        if (isPgOrZj) {
            ...
            if (hasAnyPg && hasAnyTq) {
                ...
                shred_list.push(`机巧心种(无视防御${(jq_shred * 100).toFixed(0)}%)`);
            }
        }
    }
    ...
    if(gC('b_yjy') && parseInt(gS('yjy_const'))>=1) { rs_val += 0.18; rs_list.push("耀嘉音1命(18%)"); }
    if(gC('b_ly') && parseInt(gS('ly_const'))>=1) { rs_val += 0.15; rs_list.push("琉音1命(15%)"); }
